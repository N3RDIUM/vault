...
Or better, instead of patching files, include formal levels as tags under every file. Links must stay concise. Inbox notes *must* stay orphaned, no links to and from them at all. Things that are intended to be links may be denoted with `[this]` syntax, and later converted into `[[]]` upon “adoption” (process of inbox → structure).
